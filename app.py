...
if wybór == "Chcę podać godzinę w Polsce":
    # Tworzymy pełną datę z wybraną godziną dla PL
    pl_dt = tz_polska.localize(datetime.combine(datetime.today(), wybrany_czas))
    ia_dt = pl_dt.astimezone(tz_iowa)
    
    st.info(f"🎯 Gdy w **Polsce** jest **{pl_dt.strftime('%H:%M')}**, na rancho w **Iowa** piją kawę o **{ia_dt.strftime('%H:%M')}**.")

else:
    # Tworzymy pełną datę z wybraną godziną dla Iowa
    ia_dt = tz_iowa.localize(datetime.combine(datetime.today(), wybrany_czas))
    pl_dt = ia_dt.astimezone(tz_polska)
    
    st.info(f"🎯 Gdy w **Iowa** jest **{ia_dt.strftime('%H:%M')}**, w **Polsce** jest już **{pl_dt.strftime('%H:%M')}**.")
